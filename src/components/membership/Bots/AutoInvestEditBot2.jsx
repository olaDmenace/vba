import { MinusCircleIcon, PlusCircleIcon } from '@heroicons/react/24/outline'
import React from 'react'

const AutoInvestEditBot2 = () => {
    return (
        <div>
            <div className='bg-back-back p-5 rounded-lg grid gap-5'>
                <div className='space-y-2'>
                    <h5 className='font-medium'>Auto Invest</h5>
                    <h6 className='font-medium'>Define the investment allocation</h6>
                    <p>The investment allocation refers to the amount that can be distributed across active positions.</p>
                </div>
                <div className='flex items-center gap-5'>
                    <div className='flex items-center relative rounded-lg border w-2/5 pr-3'>
                        <p className='text-lg absolute left-2'>$</p>
                        <input className='h-10 bg-transparent pl-6 w-full focus:outline-none text-right' placeholder='0' type="number" name="" id="" />
                    </div>
                    <div className='w-14 bg-primary h-7 rounded-full py-1 px-2'>
                        <div className='h-5 w-5 bg-white rounded-full'></div>
                        <input className='h-5 w-5 opacity-0' type="checkbox" name="" id="" />
                    </div>
                </div>
                <div className='flex gap-3'>
                    <MinusCircleIcon className='h-5' />
                    <input type="range" name="" id="" />
                    <PlusCircleIcon className='h-5' />
                </div>
                <div className='flex justify-between border rounded-lg px-3 py-2 w-2/3'>
                    <p>5%</p>
                    <p>10%</p>
                    <p>25%</p>
                    <p>50%</p>
                    <p>100%</p>
                </div>
            </div>
        </div>
    )
}

export default AutoInvestEditBot2